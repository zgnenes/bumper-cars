...
table.insert(elements, { label = "Onayla", value = "rentButton" })
...
ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclesRentrent_this_veh',
	...
	function(data, menu)
        ...
        { 
          ...
          align = 'top-left',
          elements = {
            {label = '<span style="color:Yellow;">Kirala şu kadara</span> <span style="color:Green;">' .. curRentPrice  .. '$ ?</span>', value = 'lol'},
			{label = '<span style="color:Green;">Onayla</span>', value = 'yes'},
            {label = 'Geri', value = 'no'},
          },
        },
        function (data2, menu2)
			...
			ESX.TriggerServerCallback('esx_vehiclesRent:buyVehicle', function(isBuyed)
				...
				ESX.ShowNotification("~g~Başarıyla Kiraladın")
				else 
				ESX.ShowNotification("~r~paran yetmiyor.")
				end
			end, vehicle)
			end
